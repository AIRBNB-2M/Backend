...
public class TourAmenityService {
    ...
    protected int fetchAndSaveFor(Accommodation acc) throws Exception {
        ...
        int linked = 0;
        for (String nm : names) {
            Amenity amenity = amenityRepository.findByName(nm)
                .orElseGet(() -> amenityRepository.save(Amenity.builder()
                    .name(nm)
                    .iconUrl(null)
                    .build()));

            if (!accommodationAmenityRepository.existsByAccommodationAndAmenity(acc, amenity)) {
                accommodationAmenityRepository.save(
                    AccommodationAmenity.builder()
                        .accommodation(acc)
                        .amenity(amenity)
                        .build()
                );
    ...
    private static final List<String> POSSIBLE_INFO_FLAG_TAGS = List.of(
    	    ...
    	    "roominternet","roomrefrigerator","roomtoiletries","roomhairdryer"
    );
